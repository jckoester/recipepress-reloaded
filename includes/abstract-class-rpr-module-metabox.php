 ...
 */
abstract class RPR_Module_Metabox extends RPR_Module {
     ...
     */
    protected function save_fields( $fields = array(), $data=NULL, $recipe = NULL, $sanitize = 'text' ){
        if( is_array( $data ) && $recipe !== NULL && $recipe->post_type === 'rpr_recipe' ){
            foreach( $fields as $key ){
		if( isset( $data[$key] ) ){
                    $old = get_post_meta( $recipe->ID, $key, true );
                    if( $sanitize == 'editor' ){
                        $new = wp_kses_post( $data[$key] );
                    } else {
