...
function rpr_update_from_1() {
  ...
  $taxonomies	 = add_taxonomy_to_array( $taxonomies, 'rpr_season', __( 'Seasons', 'recipepress-reloaded' ), __( 'Season', 'recipepress-reloaded' ) );
  $taxonomies	 = add_taxonomy_to_array( $taxonomies, 'rpr_difficulty', __( 'Difficulties', 'recipepress-reloaded' ), __( 'Difficulty', 'recipepress-reloaded' ) );
  update_option( 'rpr_taxonomies', $taxonomies );
  update_option( 'rpr_dbversion', '2' );
}

function add_taxonomy_to_array( $arr, $tag, $name, $singular, $hierarchical = false ) {
 ...
 * @since 0.8.0
 */
function rpr_update_from_rp() {
  // 1.) Register old posttype and taxonomies:
  rp_setup_post_type();
  rp_setup_ingredients();
  rp_setup_taxonomies();
  // 2.) get all old recipes
  $recipes = rp_get_old_recipes();
  ...
  foreach ( $recipes as $recipe ) {
    ...
    foreach ( $fields as $field ) {
      ...
      if ( $field == 'rpr_recipe_description' ) {
        ...
        $ings					 = rp_get_ingredients( $recipe );
    ...
    if ( $taxonomies ) {
      foreach ( $taxonomies as $taxonomy ) {
        rp_migrate_term( $recipe->ID, $taxonomy['slug'] );
      }
    }
  }
  //MISSING: delete old taxonomies:
  //require_once 'rpr_taxonomies.php';
  //$taxo = new RPR_Taxonomies($this->pluginName, $this->pluginDir, $this->pluginUrl );
  //$taxo->delete_taxonomy( $taxonomy['slug'] );
...
function rp_get_old_recipes( $orderby = 'date', $order = 'DESC', $taxonomy = '', $term = '', $limit = -1, $author = '' ) {
  $args = array(
    'post_type'		 => 'recipe',
    'posts_per_page' => -1,
  );
  ...
  if ( $orderby == 'post_title' || $orderby == 'title' || $orderby == 'name' ) {
    usort( $recipes, "compare_post_titles" );
...
function rp_get_ingredients( $post = NULL ) {
  if ( ! $post ) {
    global $post;
  }
...
function rp_empty_ingredients( $count = 5 ) {
  $ingredients = array();
...
function rp_migrate_term( $recipe_id, $slug ) {
  ...
  elseif ( $slug == 'recipe-tags' ):
    ...
    if ( ! taxonomy_exists( $new_slug ) ):
      ...
      rp_add_taxonomy( $taxonomy->name, $taxonomy->singular, $new_slug, $new_slug );
    endif;
  endif;
...
function rp_add_taxonomy( $name, $singular, $slug, $hierarchical, $edit_tag_name ) {
  $editing = false;
