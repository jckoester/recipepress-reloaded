...
( function( $ ) {
    ...
    $.fn.updateAdminPageFrameworkRepeatableSections = function( aSettings ) {
        ...
        var _oThis                = this;
        var _sSectionsContainerID = _oThis.find( '.repeatable-section-add-button' ).first().closest( '.recipepress-reloaded-sections' ).attr( 'id' );
        ...
        var _sSectionID           = _oThis.find( '.repeatable-section-add-button' ).first().closest( '.recipepress-reloaded-section' ).attr( 'id' );
        var _nCurrentSectionCount = jQuery( '#' + _sSectionsContainerID ).find( '.recipepress-reloaded-section' ).length;
    ...
    $.fn.addAdminPageFrameworkRepeatableSection = function( sSectionContainerID ) {
        ...
        if ( 'undefined' === typeof sSectionContainerID ) {
            var sSectionContainerID = $( this ).closest( '.recipepress-reloaded-section' ).attr( 'id' );
        }
        var nodeSectionContainer    = $( '#' + sSectionContainerID );
        var nodeNewSection          = nodeSectionContainer.clone(); // clone without bind events.
        var nodeSectionsContainer   = nodeSectionContainer.closest( '.recipepress-reloaded-sections' );
        var sSectionsContainerID    = nodeSectionsContainer.attr( 'id' );        
        var nodeTabsContainer       = $( this ).closest( '.recipepress-reloaded-section-tabs-contents' )
            .children( '.recipepress-reloaded-section-tabs' )
            .first();
        ...
        if ( _sMaxNumberOfSections != 0 && nodeSectionsContainer.find( '.recipepress-reloaded-section' ).length >= _sMaxNumberOfSections ) {
            var _nodeLastRepeaterButtons = nodeSectionContainer.find( '.recipepress-reloaded-repeatable-section-buttons' ).last();
        ...
        var _sSectionTabSlug = nodeNewSection.find( '.recipepress-reloaded-section-caption' ).first().attr( 'data-section_tab' );
        if ( ! _sSectionTabSlug || _sSectionTabSlug === '_default' ) {
            nodeNewSection.find( '.recipepress-reloaded-section-title' ).not( '.recipepress-reloaded-collapsible-section-title' ).hide();
        }
        ...
        if( 'function' === typeof nodeNewSection.enableAdminPageFrameworkCollapsibleButton ){ 
            nodeNewSection.find( '.recipepress-reloaded-collapsible-sections-title, .recipepress-reloaded-collapsible-section-title' ).enableAdminPageFrameworkCollapsibleButton();
        }
        ...
        $( nodeNewSection ).find( '.recipepress-reloaded-field' ).each( function( iFieldIndex ) {
            ...
            $( this ).trigger( 
                'recipepress-reloaded_added_repeatable_field', 
                [
            ...
            // 3.8.8 
            $( this ).trigger( 
                'recipepress-reloaded_repeated_field', 
                [
                    1, // call type, 0: repeatable fields, 1: repeatable sections, 
                    jQuery( nodeNewSection ).closest( '.recipepress-reloaded-sections' )    // model container
                ]
            );              
        ...
        nodeNewSection.find( '.recipepress-reloaded-fields.sortable' ).each( function() {
        ...
        if ( nodeTabsContainer.length > 0 && ! nodeSectionContainer.hasClass( 'is_subsection_collapsible' ) ) {
            ...
            nodeTabsContainer.closest( '.recipepress-reloaded-section-tabs-contents' ).createTabs( 'refresh' );
        ...
        var _incrementAttributes = function( oElement, iSectionsCount, oSectionsContainer ) {
            ...
            $( oElement ).find( 'tr.recipepress-reloaded-fieldrow, .recipepress-reloaded-fieldset, .recipepress-reloaded-fields, .recipepress-reloaded-field, table.form-table, input,textarea,select' )
                .incrementAttribute( 
                    'id', 
                    iSectionsCount,
                    _sSectionIDModel
            ...
            $( oElement ).find( '.recipepress-reloaded-fields' ).incrementAttribute( 
                'data-field_tag_id_model',
                iSectionsCount,
                _sSectionIDModel
            );
            $( oElement ).find( '.recipepress-reloaded-fields' ).incrementAttributes( 
        ...
        // @todo this may be able to be removed
            $( oElement ).find( '.recipepress-reloaded-fieldset' ).incrementAttribute( 
                'data-field_id',
                iSectionsCount,
                _sSectionIDModel
            );
    ...
    $.fn.removeAdminPageFrameworkRepeatableSection = function() {
        ...
        var nodeSectionContainer    = $( this ).closest( '.recipepress-reloaded-section' );
        var sSectionContainerID     = nodeSectionContainer.attr( 'id' );
        var nodeSectionsContainer   = $( this ).closest( '.recipepress-reloaded-sections' );
        var sSectionsContainerID    = nodeSectionsContainer.attr( 'id' );        
        var nodeTabsContainer       = $( this ).closest( '.recipepress-reloaded-section-tabs-contents' )
            .children( '.recipepress-reloaded-section-tabs' )
            .first();
        var nodeTabs                = nodeTabsContainer.children( '.recipepress-reloaded-section-tab' );
        ...
        if ( _sMinNumberOfSections != 0 && nodeSectionsContainer.find( '.recipepress-reloaded-section' ).length <= _sMinNumberOfSections ) {
            var _nodeLastRepeaterButtons = nodeSectionContainer.find( '.recipepress-reloaded-repeatable-section-buttons' ).last();
        ...
        nodeSectionContainer.fadeOut( _iFadeout, function() { 
            ...
            if ( 1 === _nodeRemoveButtons.length ) {
                ...
                var _sSectionTabSlug = nodeSectionsContainer.find( '.recipepress-reloaded-section-caption' ).first().attr( 'data-section_tab' );
                if ( ! _sSectionTabSlug || '_default' === _sSectionTabSlug ) {
                    nodeSectionsContainer.find( '.recipepress-reloaded-section-title' ).first().show();
                }                
            }            
...
var _iSectionIndex = _iIterationIndex;
            ...
            // Call the registered callback functions.
            // @deprecated  3.6.0
            // $( this ).find( '.recipepress-reloaded-field' ).each( function( iFieldIndex ) {    
...
jQuery( document ).ready( function() {
    ...
    // Adds the buttons
    jQuery( '#{$sContainerTagID} .recipepress-reloaded-section-caption' ).each( function(){
        
        jQuery( this ).show();
        
        var _oButtons = jQuery( $_sButtonsHTML );
        if ( jQuery( this ).children( '.recipepress-reloaded-collapsible-section-title' ).children( 'fieldset' ).length > 0 ) {
            _oButtons.addClass( 'section_title_field_sibling' );
        }
        var _oCollapsibleSectionTitle = jQuery( this ).find( '.recipepress-reloaded-collapsible-section-title' );
...
JAVASCRIPTS;
        return "<script type='text/javascript' class='recipepress-reloaded-section-repeatable-script'>" . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . "</script>";
    }
    static private function ___getContainerAttributes(array $aArguments, $oMsg) {
        $_aAttriubtes = array('class' => self::getClassAttribute('recipepress-reloaded-repeatable-section-buttons', empty($aArguments['disabled']) ? '' : 'disabled'),);
