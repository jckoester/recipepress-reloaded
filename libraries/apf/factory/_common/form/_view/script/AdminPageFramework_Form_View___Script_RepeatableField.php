...
// @todo For nested fields, the `find()` method should be avoided.
        $( nodeThis ).find( '.recipepress-reloaded-repeatable-field-buttons' ).attr( 'data-max', _aOptions[ 'max' ] );
        $( nodeThis ).find( '.recipepress-reloaded-repeatable-field-buttons' ).attr( 'data-min', _aOptions[ 'min' ] );
        $( nodeThis ).find( '.recipepress-reloaded-repeatable-field-buttons' ).attr( 'data-fadein', _aOptions[ 'fadein' ] );
        $( nodeThis ).find( '.recipepress-reloaded-repeatable-field-buttons' ).attr( 'data-fadeout', _aOptions[ 'fadeout' ] );
...
// @todo For nested fields, the `find()` method should be avoided.        
        ...
        var _sFieldID           = nodeThis.find( '.repeatable-field-add-button' ).first().closest( '.recipepress-reloaded-field' ).attr( 'id' );
        var _nCurrentFieldCount = jQuery( '#' + _sFieldsContainerID ).find( '.recipepress-reloaded-field' ).length;
    ...
    $.fn.addAdminPageFrameworkRepeatableField = function( sFieldContainerID ) {
        ...
        if ( 'undefined' === typeof sFieldContainerID ) {
            var sFieldContainerID = $( this ).closest( '.recipepress-reloaded-field' ).attr( 'id' );
        }

        var nodeFieldContainer  = $( '#' + sFieldContainerID );
        var nodeNewField        = nodeFieldContainer.clone(); // clone without bind events.
        var nodeFieldsContainer = nodeFieldContainer.closest( '.recipepress-reloaded-fields' );
        var _sFieldsContainerID = nodeFieldsContainer.attr( 'id' );

        // If the set maximum number of fields already exists, do not add.
        if ( ! $.fn.aAdminPageFrameworkRepeatableFieldsOptions.hasOwnProperty( _sFieldsContainerID ) ) {     
            var nodeButtonContainer = nodeFieldContainer.find( '.recipepress-reloaded-repeatable-field-buttons' );
        ...
        if ( sMaxNumberOfFields != 0 && nodeFieldsContainer.find( '.recipepress-reloaded-field' ).length >= sMaxNumberOfFields ) {
            var nodeLastRepeaterButtons = nodeFieldContainer.find( '.recipepress-reloaded-repeatable-field-buttons' ).last();
        ...
        $( nodeNewField ).find( '.recipepress-reloaded-field' ).addBack().trigger( 
            'recipepress-reloaded_repeated_field', 
            [ 
                0, // call type, 0 : repeatable fields, 1: repeatable sections
                jQuery( nodeNewField ).closest( '.recipepress-reloaded-fields' )    // model container
            ]
        );
...
// @todo find() may not be appropriate for nested fields.
        ...
        nodeFieldsContainer.children( '.recipepress-reloaded-field' ).children( '.delimiter' ).show().last().hide();
        ...
        var _incrementFieldAttributes = function( oElement, oFieldsContainer ) {
            ...
            oElement.find( '.recipepress-reloaded-field,.recipepress-reloaded-fields,.recipepress-reloaded-fieldset' ).incrementAttributes(
    ...
    $.fn.removeAdminPageFrameworkRepeatableField = function() {
        ...
        var nodeFieldContainer  = $( this ).closest( '.recipepress-reloaded-field' );
        var nodeFieldsContainer = $( this ).closest( '.recipepress-reloaded-fields' );
        var _sFieldsContainerID = nodeFieldsContainer.attr( 'id' );
        ...
        if ( sMinNumberOfFields != 0 && nodeFieldsContainer.find( '.recipepress-reloaded-field' ).length <= sMinNumberOfFields ) {
            var nodeLastRepeaterButtons = nodeFieldContainer.find( '.recipepress-reloaded-repeatable-field-buttons' ).last();
