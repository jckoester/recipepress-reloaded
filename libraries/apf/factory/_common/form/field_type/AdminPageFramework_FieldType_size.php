...
class AdminPageFramework_FieldType_select extends AdminPageFramework_FieldType {
    ...
    protected function getStyles() {
        return ".recipepress-reloaded-field-select .recipepress-reloaded-input-label-container {vertical-align: top; }.recipepress-reloaded-field-select .recipepress-reloaded-input-label-container {padding-right: 1em;}";
    }
    protected function getField($aField) {
        ...
        return $aField['before_label'] . "<div " . $this->getLabelContainerAttributes($aField, 'recipepress-reloaded-input-label-container recipepress-reloaded-select-label') . ">" . "<label for='{$aField['input_id']}'>" . $aField['before_input'] . $_oSelectInput->get($aField['label']) . $aField['after_input'] . "<div class='repeatable-field-buttons'></div>" . "</label>" . "</div>" . $aField['after_label'];
    }
}
class AdminPageFramework_FieldType_size extends AdminPageFramework_FieldType_select {
    ...
    protected function getStyles() {
        return ".recipepress-reloaded-field-size input {text-align: right;}.recipepress-reloaded-field-size select.size-field-select {vertical-align: 0px; }.recipepress-reloaded-field-size label {width: auto; } .form-table td fieldset .recipepress-reloaded-field-size label {display: inline-block;}";
    }
    protected function getField($aField) {
    ...
    private function _getNumberInputPart(array $aField, array $aBaseAttributes, $isKey, $bMultiLabels) {
        ...
        $_sLabel = $this->getElementByLabel($aField['label'], $isKey, $aField['label']);
        return "<label " . $this->getAttributes($_aSizeLabelAttributes) . ">" . $this->getElement($aField, $bMultiLabels ? array('before_label', $isKey, 'size') : array('before_label', 'size')) . ($aField['label'] && !$aField['repeatable'] ? "<span " . $this->getLabelContainerAttributes($aField, 'recipepress-reloaded-input-label-string') . ">" . $_sLabel . "</span>" : "") . "<input " . $this->getAttributes($_aSizeAttributes) . " />" . $this->getElement($aField, $bMultiLabels ? array('after_input', $isKey, 'size') : array('after_input', 'size')) . "</label>";
    }
