...
class AdminPageFramework_FieldType_system extends AdminPageFramework_FieldType {
    ...
    protected function getStyles() {
        return ".recipepress-reloaded-field-system {width: 100%;}.recipepress-reloaded-field-system .recipepress-reloaded-input-label-container {width: 100%;}.recipepress-reloaded-field-system textarea {background-color: #f9f9f9; width: 97%; outline: 0; font-family: Consolas, Monaco, monospace;white-space: pre;word-wrap: normal;overflow-x: scroll;}";
    }
    protected function getField($aField) {
        $_aInputAttributes = $aField['attributes'];
        $_aInputAttributes['class'].= ' system';
        unset($_aInputAttributes['value']);
        return $aField['before_label'] . "<div class='recipepress-reloaded-input-label-container'>" . "<label for='{$aField['input_id']}'>" . $aField['before_input'] . ($aField['label'] && !$aField['repeatable'] ? "<span " . $this->getLabelContainerAttributes($aField, 'recipepress-reloaded-input-label-string') . ">" . $aField['label'] . "</span>" : "") . "<textarea " . $this->getAttributes($_aInputAttributes) . " >" . esc_textarea($this->_getSystemInfomation($aField['value'], $aField['data'], $aField['print_type'])) . "</textarea>" . $aField['after_input'] . "</label>" . "</div>" . $aField['after_label'];
    }
    ...
    private function _getClientInfo($bGenerateInfo = true) {
        ...
        return empty($_aBrowser) ? __('No browser information found.', 'recipepress-reloaded') : $_aBrowser;
    }
    ...
    private function _getSiteInfo() {
        global $wpdb;
        return array(__('Version', 'recipepress-reloaded') => $GLOBALS['wp_version'], __('Language', 'recipepress-reloaded') => $this->getSiteLanguage(), __('Memory Limit', 'recipepress-reloaded') => $this->getReadableBytes($this->getNumberOfReadableSize(WP_MEMORY_LIMIT)), __('Multi-site', 'recipepress-reloaded') => $this->getAOrB(is_multisite(), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Permalink Structure', 'recipepress-reloaded') => get_option('permalink_structure'), __('Active Theme', 'recipepress-reloaded') => $this->_getActiveThemeName(), __('Registered Post Statuses', 'recipepress-reloaded') => implode(', ', get_post_stati()), 'WP_DEBUG' => $this->getAOrB($this->isDebugMode(), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), 'WP_DEBUG_LOG' => $this->getAOrB($this->isDebugLogEnabled(), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), 'WP_DEBUG_DISPLAY' => $this->getAOrB($this->isDebugDisplayEnabled(), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), __('Table Prefix', 'recipepress-reloaded') => $wpdb->prefix, __('Table Prefix Length', 'recipepress-reloaded') => strlen($wpdb->prefix), __('Table Prefix Status', 'recipepress-reloaded') => $this->getAOrB(strlen($wpdb->prefix) > 16, $this->oMsg->get('too_long'), $this->oMsg->get('acceptable')), 'wp_remote_post()' => $this->_getWPRemotePostStatus(), 'wp_remote_get()' => $this->_getWPRemoteGetStatus(), __('Multibite String Extension', 'recipepress-reloaded') => $this->getAOrB(function_exists('mb_detect_encoding'), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), __('WP_CONTENT_DIR Writeable', 'recipepress-reloaded') => $this->getAOrB(is_writable(WP_CONTENT_DIR), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Active Plugins', 'recipepress-reloaded') => PHP_EOL . $this->_getActivePlugins(), __('Network Active Plugins', 'recipepress-reloaded') => PHP_EOL . $this->_getNetworkActivePlugins(), __('Constants', 'recipepress-reloaded') => $this->_getDefinedConstants('user'),);
    }
    ...
    private function _getPHPInfo($bGenerateInfo = true) {
        ...
        self::$_aPHPInfo = array(__('Version', 'recipepress-reloaded') => phpversion(), __('Safe Mode', 'recipepress-reloaded') => $this->getAOrB(@ini_get('safe_mode'), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Memory Limit', 'recipepress-reloaded') => @ini_get('memory_limit'), __('Upload Max Size', 'recipepress-reloaded') => @ini_get('upload_max_filesize'), __('Post Max Size', 'recipepress-reloaded') => @ini_get('post_max_size'), __('Upload Max File Size', 'recipepress-reloaded') => @ini_get('upload_max_filesize'), __('Max Execution Time', 'recipepress-reloaded') => @ini_get('max_execution_time'), __('Max Input Vars', 'recipepress-reloaded') => @ini_get('max_input_vars'), __('Argument Separator', 'recipepress-reloaded') => @ini_get('arg_separator.output'), __('Allow URL File Open', 'recipepress-reloaded') => $this->getAOrB(@ini_get('allow_url_fopen'), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Display Errors', 'recipepress-reloaded') => $this->getAOrB(@ini_get('display_errors'), $this->oMsg->get('on'), $this->oMsg->get('off')), __('Log Errors', 'recipepress-reloaded') => $this->getAOrB(@ini_get('log_errors'), $this->oMsg->get('on'), $this->oMsg->get('off')), __('Error log location', 'recipepress-reloaded') => @ini_get('error_log'), __('Error Reporting Level', 'recipepress-reloaded') => $_oErrorReporting->getErrorLevel(), __('FSOCKOPEN', 'recipepress-reloaded') => $this->getAOrB(function_exists('fsockopen'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')), __('cURL', 'recipepress-reloaded') => $this->getAOrB(function_exists('curl_init'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')), __('SOAP', 'recipepress-reloaded') => $this->getAOrB(class_exists('SoapClient'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')), __('SUHOSIN', 'recipepress-reloaded') => $this->getAOrB(extension_loaded('suhosin'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')), 'ini_set()' => $this->getAOrB(function_exists('ini_set'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')),) + $this->getPHPInfo() + array(__('Constants', 'recipepress-reloaded') => $this->_getDefinedConstants(null, 'user'));
        return self::$_aPHPInfo;
    }
    private function _getWebServerInfo($bGenerateInfo = true) {
        return $bGenerateInfo ? array(__('Web Server', 'recipepress-reloaded') => $_SERVER['SERVER_SOFTWARE'], 'SSL' => $this->getAOrB(is_ssl(), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Session', 'recipepress-reloaded') => $this->getAOrB(isset($_SESSION), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), __('Session Name', 'recipepress-reloaded') => esc_html(@ini_get('session.name')), __('Session Cookie Path', 'recipepress-reloaded') => esc_html(@ini_get('session.cookie_path')), __('Session Save Path', 'recipepress-reloaded') => esc_html(@ini_get('session.save_path')), __('Session Use Cookies', 'recipepress-reloaded') => $this->getAOrB(@ini_get('session.use_cookies'), $this->oMsg->get('on'), $this->oMsg->get('off')), __('Session Use Only Cookies', 'recipepress-reloaded') => $this->getAOrB(@ini_get('session.use_only_cookies'), $this->oMsg->get('on'), $this->oMsg->get('off')),) + $_SERVER : '';
    }
}
