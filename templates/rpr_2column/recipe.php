...
<?php } ?>
    ...
    <div class="rpr_col1">
    	 <!-- INGREDIENTS -->
    	<h3><i class="fa fa-shopping-cart"></i><?php _e( 'Ingredients', $this->pluginName ); ?></h3>
    ...
    <div class="rpr_col2">
    	...
    	<?php if( get_the_recipe_times( $recipe_post->ID ) != "" ){ ?>
    		<h3><i class="fa fa-clock-o"></i><?php _e( 'Time', $this->pluginName ); ?></h3>
    	...
    	<?php  if( $this->option( 'recipe_tags_use_wp_categories', 1) == '1' && $this->option( 'recipe_display_categories_in_recipe' , 1 ) == '1' && get_the_recipe_taxonomy_term_list( $recipe_post->ID, 'category' ) != '' ) {?>
    		<h3><i class="fa fa-list-ul"></i><?php _e( 'Categories', $this->pluginName ); ?></h3>
    	...
    	<?php  if( $this->option( 'recipe_tags_use_wp_tags', 1) == '1' && $this->option( 'recipe_display_tags_in_recipe' , 1 ) == '1' && get_the_recipe_taxonomy_term_list( $recipe_post->ID, 'post_tag' ) != '' ) {?>
    		<h3><i class="fa fa-tags"></i><?php _e( 'Tags', $this->pluginName ); ?></h3>
		...
		?>
				<h3><i class="fa <?php echo $tax; ?>"></i><?php echo $taxonomies[$tax]['labels']['singular_name']; ?></h3>
		<?php 
    ...
    <!--- INSTRUCTIONS --->
    <h3><i class="fa fa-cogs"></i><?php _e( 'Instructions', $this->pluginName ); ?></h3>
